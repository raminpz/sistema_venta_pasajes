...
func (h *TerminalHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	pkg.WriteSuccess(w, http.StatusCreated, "terminal creada correctamente", toTerminalOutput(terminal), nil)
}
...
func (h *TerminalHandler) GetByID(w http.ResponseWriter, r *http.Request) {
	...
	pkg.WriteSuccess(w, http.StatusOK, "terminal obtenida correctamente", toTerminalOutput(terminal), nil)
}
...
func (h *TerminalHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	pkg.WriteSuccess(w, http.StatusOK, "terminal actualizada correctamente", toTerminalOutput(terminal), nil)
}
...
func (h *TerminalHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	pkg.WriteSuccess(w, http.StatusOK, "terminal eliminada correctamente", nil, nil)
}
...
func (h *TerminalHandler) List(w http.ResponseWriter, r *http.Request) {
	   ...
	   pkg.WriteSuccess(w, http.StatusOK, "terminales obtenidas correctamente", data, map[string]any{"count": len(outputs)})
}
...
func terminalIDFromRequest(r *http.Request) (int64, error) {
	...
	if err != nil || id <= 0 {
		return 0, pkg.BadRequest("invalid_terminal_id", "el id de la terminal no es válido")
	}
	return id, nil
}
...
func decodeJSONBody(r *http.Request, destination any) error {
	...
	if err := decoder.Decode(&struct{}{}); err != io.EOF {
		return pkg.BadRequest("invalid_json", "el cuerpo JSON debe contener un único objeto").WithDetails(err.Error())
	}
	return nil
}
